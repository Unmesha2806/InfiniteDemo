...
//		return "UpdateAgent.jsp?faces-redirect=true";
	 ...
	 @Override
	    public String addProviderDao(Provider provider) {
	     ...
	     String Address= provider.getAddress()+","+ provider.getCity()+ ","+ provider.getState()+","+ provider.getCountry() + ","+ provider.getPostalCode();
	     ...
	     // Check if the license number already exists
	        ...
	        if (isLicenseNumberExists(provider.getLicenseNumber())) {
	            // Display a FacesMessage for the license number existence
	            FacesMessage message = new FacesMessage("License number already exists. Please choose a different license number.");
	            context.addMessage("form:licenseNumber", message);
	            return null; // Return null or handle the situation as per your requirement
	        }
...
//	    }
	 ...
	 // Method to check if the license number already exists in the database
	    public boolean isLicenseNumberExists(String licenseNumber) {
	        ...
	        try {
	            Query query = session.createQuery("SELECT COUNT(l.licenseNumber) FROM Provider l WHERE l.licenseNumber = :licenseNumber");
	            // HQL query checking the number of records in the License entity where the licenseNumber matches the provided value (:licenseNumber).
